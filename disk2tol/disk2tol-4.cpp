...
*/
  ...
  lt0 = 4 * sizeof(uint32_t);  // bytes to copy of pixie header or pixie sums = 4 32-bit uints = size of 1 128 bit boundary
  lt1 = 2 * lt0;               // bytes to copy of pixie qdcs =  8 32-bit uints = size of 2 128-bit boundary
...
// data in spill gathered.
	...
	memcpy(&tolptr[pos],&xxxptr[ii],kk*sizeof(uint32_t));   // 128 bit copy raw data header and user-supplied data straight to tol file
	pos += kk;                                              // defined at uint32_t
...
*/	
	for (int mm=0; mm<nn; mm++){                             // write out each element of the time ordered vector
	  ...
	  switch (test){
	  case 0x01:                                             // write out 128 bits of header = time, id, status, energy, trace length
	    memcpy(&tolptr[pos],&spillData1[mm],lt0);            // 128 bit
	    pos += lt0/4;                                        // defined at uint32_t
	    //	    pos += lt0/8;                                        // defined at uint64_t
	    break;
	  case 0x02:                                             // write out 128 bits of header + 4 sum's
	    memcpy(&tolptr[pos],&spillData1[mm],lt1);            // 128 bit
	    pos += lt1/4;
	    //	    pos += lt1/8;
	    break;
	  case 0x04:                                             // write out 128 bits of header + 8 qdc's
	    memcpy(&tolptr[pos],&spillData1[mm],lt0);            // 128 bit
	    pos += lt0/4;
	    //	    pos += lt0/8;
	    ...
	    //	    pos += lt1/8;
	    break;
	  case 0x08:                                             // write out 128 bits of header + 4 sum's + 8 qdc's
...
*/	
	  if (spillData1[mm].status & 0x0020) {
	    ...
	    pos += ltx/4;                                        // uint32_t data
	    //	    pos += ltx/8;                                // uint64_t data
	  }
	}
	...
	printf ("\n Something wrong...spill data size longer than %u \n",SPILLDATA_MAX);
      ...
      /**/
      printf ("End of spill position in tol file using 32-bit words = %i \n",pos);
      //      printf ("End of spill position in tol file using 128-bit words = %i \n",pos);
 ...
 */
  ...
  uint16_t pileup, satbit, vrtbit, len, lenh, lent, crate, slot, chan, id, cfd, energy;
  struct evt n;
                                      //  spil = 83, 80, 73, 76
  tmax=0;
...
//         m.tim   = xt.timl;  
    ...
    satbit = (x0 & 0x40000000) >> 30;   // bit 30
    vrtbit = (x0 & 0x20000000) >> 29;   // bit 29
    len    = (x0 & 0x1FFE0000) >> 17;   // bit 17:27
...
*/
    ...
    if (satbit == 1) n.status += 0x40;                    // saturation bit = 
    if (vrtbit == 1) n.status += 0x80;                    // virtual bit = 
/*
    if (lenh != 4){
      printf("lenh not 4!\n");
...
//	xxxptr = (int*) mmap(0, LISTDATA_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    xxxptr = (uint32_t*) mmap(0, fsize, PROT_READ, MAP_SHARED, fd, 0);          // cannot write to file (I hope!)
...
int mmapListDataOut(char *outfile) {
    ...
    /*
     Now the file is ready to be mmapped.
     */
    tolptr = (uint32_t*) mmap(0, fileSize, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);

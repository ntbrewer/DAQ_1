...
*/
  while (ans != 100){
    ...
    case 1:                    // end program but not deg-u3
    ...
    case 6:
      printf ("Set all max currents to ... \n");
      scanf("%lf", &zzz);
      ImaxAll(zzz);
      break;
    ...
    case 9:                    // end program but not deg-u3
      OffAll();               // turn all detectors on
...
void loadShutdown(){
  int ii=0, jj=0, kk=0;
  int index, ind[2], mm=0, nn=0;
  char text[20][100], tt[10]="\0";
	  ...
	  if (strncmp(crate[ii].slot[jj].chan[kk].name,"xxx",3) ){   // identify each named detectors in hv-ln
	    ...
	    if (strcmp(lnptr->ge[mm].name, crate[ii].slot[jj].chan[kk].LNname) == 0){ // found a match between HV name and LNfill name
	      sprintf(lnptr->ge[mm].shutdown[nn],"snmpset -v 2c -L o -m %s -c guru %s outputSwitch.u%i i 3",MPOD_MIB,MPOD_IP,index);  //load shutdown message
  	      printf (lnptr->ge[mm].shutdown[nn],"snmpset -v 2c -L o -m %s -c guru %s outputSwitch.u%i i 3",MPOD_MIB,MPOD_IP,index);  //print shutdown messsage
	      nn++;
	    }
	  }
	}
	    ...
	    //	      sprintf(text[nn],"snmpset -v 2c -L o -m %s -c guru %s outputSwitch.u%i i 3",MPOD_MIB,MPOD_IP,index[nn]);




	  //	  for (mm=0; mm<20; mm++){
	    if (strlen(crate[ii].slot[jj].chan[kk].name) == 3) {
	      for(mm=0;mm<20;mm++){
	      if (strstr(crate[ii].slot[jj].chan[kk].name,lnptr->ge[mm].name) != NULL){ 
		//		sprintf(text[nn],"snmpset -v 2c -L o -m %s -c guru %s outputSwitch.u%i i 3",MPOD_MIB,MPOD_IP,index[nn]);
		sprintf(tt," %i  ",index[nn]);
		strcat(text[mm],tt);
		//		strcat(text[nn],"  ");

		printf("strings %s  %s\n",crate[ii].slot[jj].chan[kk].name,lnptr->ge[mm].name);  // store message of shurdown command
		printf("mm=%i  %s\n",mm,text[mm]);  // store message of shurdown command
		ind[nn]=mm;               // store index of lnfill detectors
/*
		for (nn=0; nn<4; nn++){
		  strcpy(lnptr->ge[mm].shutdown[nn],text);
		}
*/
	      }
	      }
	    }
	    else if  (strlen(crate[ii].slot[jj].chan[kk].name) == 4) {
	      if (strncmp(crate[ii].slot[jj].chan[kk].name,lnptr->ge[mm].name,3)) {
		sprintf(text[nn],"snmpset -v 2c -L o -m %s -c guru %s outputSwitch.u%i i 3",MPOD_MIB,MPOD_IP,index[nn]);

		printf("%s\n",text[nn]);  // store message of shurdown command
		ind[nn]=mm;               // store index of lnfill detectors
/*
		for (nn=0; nn<4; nn++){
		  strcpy(lnptr->ge[mm].shutdown[nn],text);
...
*/
  ...
  //  for (ii=0; ii<4; ii++) {
  /*
    for (pp=0; pp<6; pp++){      // maximum 6 clovers
      count = 0;
      for (mm=0; mm<nn; mm++){
	if (pp == ind[mm]) {
	  strcpy(lnptr->ge[ind[mm]].shutdown[count++],text[index[mm]]);
	}
      }
    }
      //  }
    ...
    strcpy (lnptr->ge[mm].shutdown[

    if (strstr(lnptr->ge[mm].name,   != NULL) 
    //    if (strcmp(lnptr->ge[nn].name,
    printf("%i - %s \n",index[mm],text[mm]);
  }
...
void reportStatus(){
  ...
  printf ("----------------------------------------------------------------------------\n");
  printf (" Det   MPOD   OnOff   Vset    Vred   Temp   Status      \n");
  printf ("----------------------------------------------------------------------------\n");
  for (ii=0; ii< 1; ii++){
    for (jj=0; jj<4; jj++){
      for (kk=0; kk<16; kk++){
...
*/
	  for (mm=0; mm<20; mm++){
	    if (strlen(crate[ii].slot[jj].chan[kk].name) == 3) {
	      if (strncmp(crate[ii].slot[jj].chan[kk].name,lnptr->ge[mm].name,2)) kelvin=lnptr->ge[mm].rtd;
	    }
	    else if  (strlen(crate[ii].slot[jj].chan[kk].name) == 4) {
	      if (strncmp(crate[ii].slot[jj].chan[kk].name,lnptr->ge[mm].name,3)) kelvin=lnptr->ge[mm].rtd;
	    }
	  } // end of mm-for
	  printf ("%4s   %s     %i   %0.1lf   %0.1lf   %0.1lf   %s \n",
...
int menu(){
  ...
  printf ("----------------------------------------------------------------------------\n");
  printf ("1 - All (conf file)  | 11 -                    | Grow-in     = \n");//%li ms\n",igrow);
  printf ("2 - Voltages         | 12 - Voltage            | Decay-out   = \n");//%li ms\n",idecay);
  printf ("3 - Max voltages     | 13 - Max Voltage        | MTC move    = \n");//%li ms \n",mtc);
  printf ("4 - Ramp voltages    | 14 - Ramp Voltage       | MTC move    = \n");//%li ms \n",mtc);
  printf ("5 - Current trips    | 15 - Current trip       | Ampl. (P-P) = \n");//%li mV\n",amplitude);
  printf ("6 - Max currents     | 16 - Max current        | Ampl. (P-P) = \n");//%li mV\n",amplitude);
  printf ("7 - On/Off           | 17 - On/Off             | Ampl. (P-P) = \n");//%li mV\n",amplitude);
  printf ("8 - LNfill shutdowns | 18 - LNfill shutdown    | Ampl. (P-P) = \n");//%li mV\n",amplitude);
  printf ("9 - Reset alarms     | 19 - Reset alarm        | Ampl. (P-P) = \n");//%li mV\n",amplitude);
  printf ("----------------------------------------------------------------------------\n");
  printf ("0 - End program      | 20 -                    | Ampl. (P-P) = \n");//%li mV\n",amplitude);
...
//    mm = sscanf (line,"%s",&xname);
//    if (strcmp(xname,"MTAS") != 0) {
    mm = sscanf (line,"%i %s %c %i %f %f %f %f %i",&pp,&xname,&u,&crslch,&volts,&vmx,&imx,&vrmp,&itrp);
    //    printf ("mm = %i, pp = %i\n",mm,pp);
...
void VsetAll(){ //float vconst) {
  ...
  vset = xval;    //atof(gtk_entry_get_text(GTK_ENTRY(entry)));      
  for (ii=0; ii< 1; ii++){
    for (jj=0; jj<4; jj++){
      for (kk=0; kk<16; kk++){
	if (strncmp(crate[ii].slot[jj].chan[kk].name,"xxx",3) ){
...
void VmaxAll() {
  ...
  vmax = xval;     //atof(gtk_entry_get_text(GTK_ENTRY(entry)));       
  printf("vmax=>%f\n",vmax);
...
void VrampAll() {
  ...
  vramp = xval;    //atof(gtk_entry_get_text(GTK_ENTRY(entry)));       
  printf("vramp=>%f\n",vramp);

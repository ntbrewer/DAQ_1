...
#define IBOOTBAR_IP "192.168.13.240"  // ORNL address: mask 255.255.255.0   Subnet 192.168.13.240 for ANL set-up
...
*/
  if (setget == 1) {
    sprintf (com,"snmpset -r 15 -t 5 -v 1 -L o -m %s -c private %s ",IBOOTBAR_MIB,IBOOTBAR_IP);
  } 
  else if (setget == 0){
    sprintf (com,"snmpget -r 15 -t 5 -v 1 -L o -m %s -c public  %s ",IBOOTBAR_MIB,IBOOTBAR_IP);

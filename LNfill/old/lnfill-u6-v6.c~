...
void closeDet (int ii){
    ...
    strcpy(lnptr->ge[ii].status,"Closed");
...
*/
    ...
    */
	...
	kk=0;                                     // reset counter to 0...false readings occur from splashes of LN
      ...
      sleep(5);
      if (lnptr->command != 8) return;     // another interupt signal was sent ...go to users needs to manual get things right
    }
   
    //    closeMani();
    for (ii=0; ii<activemax; ii++){
      openDet(active[ii]);
      if (lnptr->command != 8) return;  
    }
...
*/
	...
	curtime = time(NULL);
	if (actkk[ii] <= 4 && actkk[ii] != 9) {       // set to 9 when detector finished filling
	  if(lnptr->ge[active[ii]].oflo < lnptr->ge[active[ii]].olimit){   // check if oflo cold
	    ...
	    if (actkk[ii] > 3) {                  // read overflo values 4 times to be sure they are cold
	      ...
	      lnptr->ge[active[ii]].next = curtime + lnptr->ge[active[ii]].interval;   // set next filling time
      ...
      if (lnptr->command != 8) return;     // another interupt signal was sent ... user needs to manually get things right
...
void sendEmail(){
  ...
  char com[150];
  char res[200];
  int ii=0, maxAddress=0;

  for (ii=0; ii< 10; ii++){
    if (strlen(lnptr->email[ii]) > 4)maxAddress++;
    }

  for (ii=0; ii< maxAddress; ii++){
    sprintf (com,"mail -s \"Liquid nitrogen filling failure\" %s < include/fill-failed.txt",lnptr->email[ii]);
    //                     1         2         3         4         5         6         7         8
    //            12345678901234567890123456789012345678901234567890123456789012345678901234567890 + 40 for email addresses
  //  strcat(com,cmd);                             // add file to send to the mail system call
    printf("%s\n",com);

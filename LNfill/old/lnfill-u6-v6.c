...
*/
  ...
  for (ii=0; ii<20; ii++){
	...
	if (lnptr->command == 7 || lnptr->command == 8) strcpy(lnptr->ge[ii].status,"OK-FILL");
      }
    }
  }
  lnptr->tank.rtd = readRTD(lnptr->tank.chanRTD);
...
void openDet (int ii){
    ...
    strcpy(lnptr->ge[ii].status,"OK - FILL");
...
*/
    ...
    */
    strcpy(lnptr->comStatus,"FILLING ALL");   
    for (ii=0;ii<6;ii++){
        active[ii]=0;
        actkk[ii]=0;
...
void sendEmail(){
  ...
  */
  for (ii=0; ii< lnptr->maxAddress; ii++){
    ...
    sprintf (com,"mail -s \"LN problem \" %s",lnptr->ge[ii].email);
    //                     1         2         3         4         5         6         7         8
    ...
    for (ii=0; ii< 20; ii++){
      if (lnptr->ge[ii].onoff == 1){
	fprintf(fp,"%i - %s - %s \n",ii,lnptr->ge[ii].name,lnptr->ge[ii].status);
      }
    }
    fprintf(fp,".");
    fflush(fp);  

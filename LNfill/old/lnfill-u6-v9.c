...
*/
  ...
  for (ii=0; ii<20; ii++){
	...
	strcpy(lnptr->ge[ii].status,"ALARM");                  // over limit
	// for (jj=0; jj<4; jj++){
	  //	  if (strlen(lnptr->ge[ii].shutdown[jj]) < 4) snmp(set2,lnptr->ge[ii].shutdown[jj],cmdResult);         //!@#$% issue HV shutdown commands
	  if (ii == 0) {
	  ...
	  if (ii == 5) {
	    snmpShutdown(101,cmdResult);         //!@#$% issue HV shutdown commands
	    snmpShutdown(102,cmdResult);         //!@#$% issue HV shutdown commands
	    snmpShutdown(103,cmdResult);         //!@#$% issue HV shutdown commands
	    snmpShutdown(104,cmdResult);         //!@#$% issue HV shutdown commands
	  }
	  if (ii == 10) {
	    snmpShutdown(201,cmdResult);         //!@#$% issue HV shutdown commands
	    snmpShutdown(202,cmdResult);         //!@#$% issue HV shutdown commands
	    snmpShutdown(203,cmdResult);         //!@#$% issue HV shutdown commands
	    snmpShutdown(204,cmdResult);         //!@#$% issue HV shutdown commands
	  }
      }
...
//void snmp(int setget, char *cmd, char *cmdResult) {
  ...
  sprintf (com,"snmpset -v 2c -L o -m %s -c guru %s outputSwitch.u%i i 3",MPOD_MIB,MPOD_IP,index);     //!@#$%^& 
...
/*
   setget flag chooses if this is a 0=read (snmpget) or 1=write (snmpset) or 2-shutdown with MPOD MIB and IP from HV control
*/
  if (setget == 1) {

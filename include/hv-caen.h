...
#define HVCAENDATAPATH "../data/hvcaen.bin"            // user data for input to xia2disk
#define HVCAENDATASIZE sizeof(struct hvcaen)
...
struct hvchan {
  ...
  unsigned short int onoff;  // Pw
  unsigned short int powUp; // POn   = when powered up do voltages turn on immediately
  unsigned short int kill;   // PDwn  = kill or ramp down on trip
  unsigned short int status; // Status
  float iMeas;   // uA
  float vMeas;   // Volts
  float vSet;    // Volts
...
struct hvcaen {
  ...
  int com4;     // commands between daq to turn off

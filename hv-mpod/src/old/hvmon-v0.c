/*
  ...
  To be compiled with:                                           
     gcc -lm -lcaenhvwrapper -o hvmon hvmon-v0.c
...
*/
  ...
  while(hvptr->com0 != -1) {
    ...
    //       	mpodHV();
    //	caenHV();
    printf (" firmware   %s   to   %s \n",(char *)firmwareMin,(char *)firmwareMax);
	printf (" number of slots %u \n",numOfSlots);
	for (ii=0; ii<numOfSlots;ii++){
	  printf (" chan: %i model: %i  desc: %i  serial number: %u \n",ii,model[ii],descModel[ii],serNumList[ii] );
	    //char *model, *descModel;
	    //unsigned short numOfSlots;
	    //unsigned short *numOfChan, *serNumList;
	    //unsigned char  *firmwareMin, *firmwareMax;
 ...
 firmware      to    
 number of slots 16 
 chan: 0 model: 0  desc: 0  serial number: 0 
 chan: 1 model: 0  desc: 0  serial number: 0 
 chan: 2 model: 65  desc: 32  serial number: 272 
 chan: 3 model: 49  desc: 50  serial number: 0 
 chan: 4 model: 53  desc: 52  serial number: 0 
 chan: 5 model: 51  desc: 32  serial number: 144 
 chan: 6 model: 53  desc: 67  serial number: 122 
 chan: 7 model: 0  desc: 104  serial number: 156 
 chan: 8 model: 0  desc: 32  serial number: 129 
 chan: 9 model: 0  desc: 78  serial number: 165 
 chan: 10 model: 65  desc: 101  serial number: 0 
 chan: 11 model: 49  desc: 103  serial number: 0 
 chan: 12 model: 53  desc: 46  serial number: 722 
 chan: 13 model: 51  desc: 32  serial number: 0 
 chan: 14 model: 53  desc: 51  serial number: 0 
 chan: 15 model: 0  desc: 46  serial number: 0 
...
*/    
	...
	//	  result = CAENHV_DeinitSystem(hvptr->xx[indexMax].caenH);   // only opens 1527 CAEN systems
	//	if (result != 0) printf("Error deattaching to CAEN system %s \n",hvptr->xx[indexMax].ip);
	//	else printf ("Successful logout!\n");

		//          	result = CAENHV_GetCrateMap(hvptr->xx[indexMax].caenH, (unsigned short *) numOfSlots, (unsigned short **) numOfChan, (char **) model, (char **)descModel, (unsigned short **) serNumList, (unsigned char **) firmwareMin, (unsigned char **)firmwareMax);   // need to get the crate info in order to access
	//	if (result != 0) printf("Error mapping CAEN system %s \n",hvptr->xx[indexMax].ip);
...
*/
  ...
  for (ii=0; ii<indexMax; ii++){
    if (hvptr->xx[ii].type == 1) {
      ...
      result = CAENHV_GetChParam(hvptr->xx[ii].caenH, hvptr->xx[ii].slot,"VMON", 1, &hvptr->xx[ii].chan, (void *)&val);
      if (result != 0) printf("Error getting VMON from CAEN system %s \n",hvptr->xx[indexMax].ip);
      else hvptr->xx[ii].vMeas = val;           // measured voltage
      ...
      result = CAENHV_GetChParam(hvptr->xx[ii].caenH, hvptr->xx[ii].slot, "RUP", 1, &hvptr->xx[ii].chan, (void *)&val);

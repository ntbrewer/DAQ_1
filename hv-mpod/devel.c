...
void setHVmpod(int sg) {
  ...
  for (ii=0; ii<indexMax; ii++){
    ...
    if (hvptr->xx[ii].type == 0) {
      sprintf(cmd,"outputSwitch.u%i i %i",hvptr->xx[ii].chan,sg); 
      snmp(setget,ii,cmd,cmdResult);     //mpodGETguru(cmd, cmdResult);     // read the set voltage
      hvptr->xx[ii].onoff = readInt(cmdResult);
    }
  }
  printf (".... finished \n");  
  return;
...
*/
  if (setget == 1) {
    ...
    sprintf (com,"snmpget -OqU -v 2c -M /usr/share/snmp/mibs -m %s -c guru %s ",MPOD_MIB,hvptr->xx[ii].ip);
